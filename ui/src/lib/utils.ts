...
export function setStackRefreshCookie(refreshToken: string) {
  ...
  const isIvaqtechDomain = window.location.hostname.endsWith('.ivaqtech.com');
  const cookieDomainPart = isIvaqtechDomain ? '; domain=.ivaqtech.com' : '';
...
export async function impersonateAsSuperadmin(params: {
  ...
  // (served from the target sub-domain, e.g. app.ivaqtech.com). The route will set the
  ...
  // superadmin.ivaqtech.com we want to switch to app.ivaqtech.com. For any other domain
